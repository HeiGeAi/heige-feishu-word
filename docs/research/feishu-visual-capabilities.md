...
这是本项目的设计判断：图示需要和前后的正文一起成立。反复插入大幅开场海报、独立背景或重复标题，会增加块之间的断裂感。当前六套样式共享白底，以主辅双色、类别色和对应浅底组织章节、指标与图形。
...
白色宿主解决正文与图形背景割裂的问题，也为更鲜明的类别色提供共同底色。本轮设计继续保持紧凑图形与文末明细，通过主辅色交替、指标实底与浅底、阶段配色增加辨识度。这是项目的设计迭代，未增加新的飞书接口能力。

每套主题定义八种浓色 `palette` 和八种配对浅色 `tints`。首项 KPI 采用 `primary` 实底白字，其他指标沿前四色配对使用浅底与深色数字。单系列条形图按类别着色，多系列图按系列保持颜色一致；流程阶段保留深色正文，以浅底、浓色编号和箭头区分步骤。具体色值来自项目 [`themes.py`](../../src/heige_feishu_word/themes.py)。

颜色承担类别与层级作用。成功、提醒和风险语义来自作者提供的 `callout.tone`、`timeline.status`，不会从任意 KPI 的颜色推断趋势。直接标签、精确读数、编号标记和不同折线线型同时保留，降低对单纯辨色的依赖。

原生 XML 使用平台允许的预设色名及对应浅色变体。项目将章节主辅色和类别色映射到 `blue`、`orange`、`green`、`purple` 等预设色系；精确 HEX 色值用于 SVG 和 HTML。两类输出能保持相近色系，不能据此承诺原生命名颜色与 HEX 完全相同。[官方原生颜色规则](https://github.com/larksuite/cli/blob/main/skills/lark-doc/references/lark-doc-xml.md)

亮 `secondary` 与深色 `palette[1]` 的用途不同：前者用于图形强调，后者用于第二类别的可读文字和数据系列。明黄、琥珀等明亮辅助色不会直接被当作白底小字的默认颜色。
...
| 表格 | `table`、`th`、`td` | 按章节预设色映射浅色表头，保留精确数据和同口径方案比较 |
...
| 哪个阶段发生流失 | 漏斗图 | 后一阶段数值不大于前一阶段，宽度对应数值 |
...
项目避免用截断柱轴或 3D 饼图制造差异。填色标识类别，长度、面积与直接读数继续遵循数据关系。空白数据不补造成零，超出文字容量时明确报错，要求拆图或调整单位。图形保真与业务判断正确是两项不同的检查。

## 六套白页语法

| 主题 ID | 名称 | 配色与沟通重点 |
| :--- | :--- | :--- |
| `atelier-bone` | 海蓝鎏金简报 | 海蓝与鎏金为主，松石绿与莓红补充预算类别 |
| `grid-bureau` | 蓝橙经营复盘 | 钴蓝与橘色区分系列，青绿与紫色补充渠道类别 |
| `nocturne-teal` | 孔雀琥珀简报 | 孔雀青与琥珀黄为主，宝蓝与莓色区分阶段和读数 |
| `broadsheet` | 莓果研究速递 | 莓红与靛蓝交替组织证据，湖蓝和翡翠绿补充类别 |
| `moxi-void` | 朱砂石青备忘 | 朱砂与石青形成方案对照，竹青和赭金补充条件 |
| `soundwave-wrapped` | 洋红明黄提案 | 洋红主读数、明黄浅色面与宝蓝辅助，组织发布与转化 |
